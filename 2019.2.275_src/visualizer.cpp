...
extern bool showEmotionBar;  // HPE ADD allows us to use showEmotionBar from main.cpp 
...
void EmotionBarVisualizer::draw(cv::Mat& img, std::map<std::string, float> emotions, cv::Point org, cv::Scalar fgcolor, cv::Scalar bgcolor) {
	...
	/* HPE ADD 
	float biggest = 0;
	size_t last_emotion = -1;
	for (size_t i = 0; i < emotionNames.size(); i++) {
		if (emotions[emotionNames[i]] > biggest) {
			last_emotion = i;
			biggest = emotions[emotionNames[i]];
		}

	}
	std::cout << "EMOTION:" << emotionNames[last_emotion] << std::endl;
	// END HPE */
}
...
void Visualizer::drawFace(cv::Mat& img, Face::Ptr f, bool drawEmotionBar) {
	...
	if (f->isEmotionsEnabled()) {
		...
		out << "," << emotion.first;
		std::cout << "EMOTION:" << emotion.first << std::endl;
	}
	/* HPE REMOVE  This is the emotion above the face detected
	cv::putText(img,
		out.str(),
...
void Visualizer::draw(cv::Mat img, std::list<Face::Ptr> faces) {
	...
	for (auto&& face : faces) {
		if (::showEmotionBar) { // HPE REMOVE emotionVisualizer) {
